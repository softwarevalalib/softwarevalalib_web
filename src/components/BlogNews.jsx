...
export default function BlogNews() {
  return (
    <section className="section-padding bg-slate-50 transition-colors duration-300">
        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {POSTS.map((post, i) => (
            <Reveal key={post.id} delay={i * 0.08}>
              <article className="lift h-full rounded-2xl overflow-hidden bg-white border border-slate-100 flex flex-col">
                <div className="relative h-44 overflow-hidden">
                  ...
                  />
                  <span className="absolute top-3 left-3 text-xs font-semibold bg-[#c10020] text-white px-3 py-1 rounded-full">
                    {post.category}
                  </span>
                </div>
