...
function ServicesContent() {
  return (
    <section className="bg-slate-50 pb-16 sm:pb-20 transition-colors duration-300">
      <div className="section-container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        {Services.map((service, index) => (
          <Reveal key={service.id} delay={index * 0.08}>
            <article className="lift h-full bg-white rounded-2xl p-6 shadow-sm border border-slate-100 group">
              <div className="flex items-center gap-4 pb-4">
                <span className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-[#c10020]/10 text-[#c10020] group-hover:bg-[#c10020] group-hover:text-white transition-all duration-300">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {service.features.map((feature, i) => (
                  <span
                    key={i}
                    className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-700 transition-colors duration-300 group-hover:bg-[#c10020]/10 group-hover:text-[#a0001a] "
                  >
                    {feature}
                  </span>
                ))}
              ...
              <Link
                to="/contact"
                className="inline-flex items-center justify-center rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#c10020] transition-all duration-300"
              >
                Get Started
              </Link>
            </article>
