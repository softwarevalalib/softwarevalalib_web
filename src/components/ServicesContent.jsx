      ...
      <div className="section-container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        {Services.map((service, index) => (
          <Reveal key={service.id} delay={index * 0.08}>
            <article className="h-full bg-white rounded-2xl p-6 shadow-sm border border-slate-100 card-hover group">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {service.features.map((feature, i) => (
                  <span
                    key={i}
                    className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-700 transition-colors duration-300 group-hover:bg-orange-50 group-hover:text-orange-700"
                  >
                    {feature}
                  </span>
                ))}
